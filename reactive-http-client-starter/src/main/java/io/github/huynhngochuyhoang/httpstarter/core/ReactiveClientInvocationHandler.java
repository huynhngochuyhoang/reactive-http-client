...
import io.github.huynhngochuyhoang.httpstarter.core.SubscriptionReportingState.Attempt;
import io.github.huynhngochuyhoang.httpstarter.core.SubscriptionReportingState.FinalRequestObservation;
import io.github.huynhngochuyhoang.httpstarter.core.SubscriptionReportingState.TerminalSignal;
...
public class ReactiveClientInvocationHandler implements InvocationHandler {
    ...
    public ReactiveClientInvocationHandler(
        ...
        this(webClient, metadataCache, argumentResolver, errorDecoder, clientConfig, clientName, null,
                ...
                LocalResponseCacheManager.lazy(applicationContext.getClassLoader()), null, null);
    }
    ...
    public static ReactiveClientInvocationHandler create(
            ...
            ReactiveHttpClientJsonCodec jsonCodec,
            ReactiveHttpClientProperties.ObservabilityConfig observabilityConfig) {
        return create(webClient, metadataCache, argumentResolver, errorDecoder, clientConfig, clientName,
                clientInterface, applicationContext, resilienceOperatorApplier, jsonCodec,
    ...
    public ReactiveClientInvocationHandler(
        ...
        this(webClient, metadataCache, argumentResolver, errorDecoder, clientConfig, clientName,
                clientInterface, applicationContext, resilienceOperatorApplier, jsonCodec,
                observabilityConfig, LocalResponseCacheManager.lazy(applicationContext.getClassLoader()), null, null);
    }

    ReactiveClientInvocationHandler(
            WebClient webClient,
            MethodMetadataCache metadataCache,
            RequestArgumentResolver argumentResolver,
            DefaultErrorDecoder errorDecoder,
        ...
        this(webClient, metadataCache, argumentResolver, errorDecoder, clientConfig, clientName,
                clientInterface, applicationContext, resilienceOperatorApplier, jsonCodec,
                observabilityConfig, responseCacheManager, null, null);
    }
    ...
    public static ExchangeFilterFunction finalRequestObservationFilter() {
        return (request, next) -> {
            request.attribute(OBSERVED_REQUEST_URL_ATTRIBUTE)
                    .ifPresent(target -> {
                        if (target instanceof Attempt attempt) {
                            attempt.observeRequestUrl(request.url());
                        } else if (target instanceof AtomicReference<?> reference) {
                            @SuppressWarnings("unchecked")
                            AtomicReference<URI> requestUrl = (AtomicReference<URI>) reference;
                            requestUrl.set(request.url());
                        }
                    });
            request.attribute(FINAL_REQUEST_OBSERVATION_ATTRIBUTE)
                    .ifPresent(target -> {
                        FinalRequestObservation observation = new FinalRequestObservation(
                                request.method().name(), request.url(), copyRequestHeaders(request.headers()));
                        if (target instanceof Attempt attempt) {
                            attempt.observeFinalRequest(observation);
                        } else if (target instanceof AtomicReference<?> reference) {
                            @SuppressWarnings("unchecked")
                            AtomicReference<FinalRequestObservation> finalRequest =
                                    (AtomicReference<FinalRequestObservation>) reference;
                            finalRequest.set(observation);
                        }
                    });
    ...
    private Object invokeResolved(Object proxy,
                                  ...
                                  Long cacheMaximumDecodedResponseBytes) {
        ...
        if (plan.returnsFlux()) {
            Flux<?> flux = usesSubscriptionState
                    ...
                    cacheMaximumDecodedResponseBytes)
                ...
                flux = Flux.deferContextual(ctx -> {
                    AtomicReference<Map<String, List<String>>> inboundHeadersRef = new AtomicReference<>(
                            ctx.hasKey(InboundHeadersWebFilter.INBOUND_HEADERS_CONTEXT_KEY)
                                    ...
                                    : Map.of());
                    SubscriptionReportingState state = subscriptionState(ctx);
                    return capturedFlux
    ...
    private Mono<WebClient.RequestHeadersSpec<?>> statelessRequestHeadersSpec(
        ...
        return Mono.deferContextual(context -> {
            ...
            SerializedRequestBody requestBody = preparedSerializedRequestBody != null
                    ...
                    : new SerializedRequestBody(preparedResolved.body(), preparedResolved.body(), null, null);
    ...
    private WebClient.RequestHeadersSpec<?> buildRequestHeadersSpec(
        ...
        WebClient.RequestBodySpec preparedRequestSpec = requestClient
                ...
                .uri(uriBuilder -> DeclarativeRequestUri.build(
                        ...
                        resolved,
                        "Request URI for client [" + clientName + "] method " + plan.method().toGenericString()));
        if (!hasAcceptHeader) {
        ...
        if (serializedRequestBody.authContextValidator() != null) {
            preparedRequestSpec = preparedRequestSpec.attribute(
                    AuthRequest.AUTH_CONTEXT_VALIDATOR_ATTRIBUTE, serializedRequestBody.authContextValidator());
        }
        ...
        if (attempt != null) {
            requestHeadersSpec = requestHeadersSpec.attribute(OBSERVED_REQUEST_URL_ATTRIBUTE, attempt);
            requestHeadersSpec = requestHeadersSpec.attribute(FINAL_REQUEST_OBSERVATION_ATTRIBUTE, attempt);
            requestHeadersSpec = requestHeadersSpec.attribute(
                    AuthRequest.REQUEST_OBSERVATION_RESET_ATTRIBUTE,
                    (Runnable) () -> state.resetAttemptEvidence(attempt));
        }
        return configureNativeRequest(requestHeadersSpec, timeoutMs, shouldApplyResponseTimeout, attempt);
    }
        ...
        return Flux.deferContextual(context -> {
            ...
            return requestHeadersSpecMono.flatMapMany(requestHeadersSpec -> {
                ...
                return requestHeadersSpec.exchangeToFlux(clientResponse -> {
                    ...
                    LocalResponseCacheManager.ResponseMetadata metadata = recordCacheResponse(
                            cacheResponseMetadata, clientResponse.statusCode(), responseHeaders,
                            cacheMaximumDecodedResponseBytes);

                    if (clientResponse.statusCode().isError()) {
                        return decodeErrorResponse(clientResponse).flatMapMany(Mono::error);
                    }
                    return Flux.from(successResponseHandler.apply(observeDecodedResponseBytes(clientResponse, metadata)));
                }).doFinally(ignored -> state.clearActiveAttempt(attempt));
            });
        });
    }
    ...
    private <T> Flux<T> exchangeStateless(
        ...
        return requestHeadersSpecMono.flatMapMany(requestHeadersSpec -> requestHeadersSpec.exchangeToFlux(clientResponse -> {
            LocalResponseCacheManager.ResponseMetadata metadata = null;
            if (cacheResponseMetadata != null) {
                metadata = recordCacheResponse(
    ...
    private LocalResponseCacheManager.ResponseMetadata recordCacheResponse(
            AtomicReference<LocalResponseCacheManager.ResponseMetadata> cacheResponseMetadata,
        ...
        LocalResponseCacheManager.DecodedResponseBytes decodedResponseBytes =
                maximumDecodedResponseBytes != null && !statusCode.isError()
                        ? new LocalResponseCacheManager.DecodedResponseBytes(maximumDecodedResponseBytes)
                        : null;
        LocalResponseCacheManager.ResponseMetadata metadata = new LocalResponseCacheManager.ResponseMetadata(
                statusCode.value(), responseHeaders, true, decodedResponseBytes);
        cacheResponseMetadata.set(metadata);
        return metadata;
    }

    private ClientResponse observeDecodedResponseBytes(
            ClientResponse response, LocalResponseCacheManager.ResponseMetadata metadata) {
        if (metadata == null || metadata.decodedResponseBytes() == null) {
            return response;
        }
        LocalResponseCacheManager.DecodedResponseBytes measurement = metadata.decodedResponseBytes();
        return response.mutate()
                .body(body -> body
                        .doOnNext(buffer -> measurement.add(buffer.readableByteCount()))
                        .doOnComplete(measurement::complete)
                        .doOnError(ignored -> measurement.unavailable())
                        .doOnCancel(measurement::unavailable))
                .build();
    }
            ...
            return requestHeadersSpecMono.flatMap(requestHeadersSpec -> {
                ...
                WebClient.ResponseSpec responseSpec = requestHeadersSpec.retrieve()
                        .onStatus(HttpStatusCode::isError, response -> {
                            attempt.recordResponse(response.statusCode(), copyHeaders(response));
                ...
                return entity
                        ...
                        .doOnNext(responseEntity -> {
                            attempt.recordResponse(
                                    responseEntity.getStatusCode(), copyHeaders(responseEntity.getHeaders()));
                        })
                        .doFinally(ignored -> state.clearActiveAttempt(attempt));
            });
        });
    ...
    private Mono<?> applyResilienceMono(Mono<?> mono,
        ...
        if (policy.rateLimiter().active()) {
            mono = applyRateLimiterMono(mono, policy.rateLimiter().instanceName());
        }
        if (policy.circuitBreaker().active()) {
            mono = applyCircuitBreakerMono(mono, policy.circuitBreaker().instanceName());
        }
        if (policy.bulkhead().active()) {
            mono = applyBulkheadMono(mono, policy.bulkhead().instanceName());
        }
        return mono;
    }
    ...
    private Flux<?> applyResilienceFlux(Flux<?> flux,
        ...
        if (policy.rateLimiter().active()) {
            flux = applyRateLimiterFlux(flux, policy.rateLimiter().instanceName());
        }
        if (policy.circuitBreaker().active()) {
            flux = applyCircuitBreakerFlux(flux, policy.circuitBreaker().instanceName());
        }
        if (policy.bulkhead().active()) {
            flux = applyBulkheadFlux(flux, policy.bulkhead().instanceName());
        }
        return flux;
    }
    ...
    private LogicalCallTimeoutException logicalCallTimeout(SubscriptionReportingState state, long timeoutMs) {
        HttpClientFailureStage failureStage = null;
        Attempt activeAttempt = state.activeAttempt();
        if (activeAttempt != null && activeAttempt.number() == state.attemptCount()) {
            if (activeAttempt.responseStatus() != null) {
    ...
    private WebClient.RequestHeadersSpec<?> configureNativeRequest(
            ...
            boolean shouldApplyResponseTimeout,
            Attempt attempt) {
        if (!shouldApplyResponseTimeout && attempt == null) {
            return requestHeadersSpec;
        }
        return requestHeadersSpec.httpRequest(httpRequest -> {
            if (attempt != null) {
                attempt.observeRequestUrl(httpRequest.getURI());
            }
    ...
    private Mono<SerializedRequestBody> serializeRequestBodyForAuth(Object body, String contentTypeHeader) {
        if (body == null) {
            return Mono.just(new SerializedRequestBody(null, null, null, null));
        }
        if (!clientConfig.hasAuthConfigured()) {
        ...
        if (body instanceof byte[] bytes) {
            return Mono.just(new SerializedRequestBody(body, bytes, bytes, null));
        }
        if (body instanceof String text) {
            return Mono.just(new SerializedRequestBody(
                    body, text, text.getBytes(rawBodyCharset(contentTypeHeader)), null));
        }
    ...
    private Mono<SerializedRequestBody> prepareCacheLoadBody(
            RequestArgumentResolver.ResolvedArgs resolved,
            CacheBodyPreparation bodyPreparation) {
        if (bodyPreparation.requestBody() != null) {
            return Mono.just(bodyPreparation.requestBody());
        }
        return serializeRequestBodyForAuth(
    ...
    private Mono<CacheAuthorization> authorizeCacheLookup(
        ...
        return Mono.defer(() -> {
            ...
            if (requestBody.authContextValidator() != null) {
    ...
    private void validateCacheAuthContentType(String expectedContentType, AuthContext authContext) {
        ...
        final String replacementContentType;
        try {
            replacementContentType = canonicalCacheContentType(MediaType.parseMediaType(authContentType));
        } catch (Exception error) {
            throw cacheAuthContentTypeReplacement(error);
        }
        if (!Objects.equals(expectedContentType, replacementContentType)) {
            throw cacheAuthContentTypeReplacement(null);
        }
    }

    private IllegalStateException cacheAuthContentTypeReplacement(Exception cause) {
        String message = "AuthProvider for cache-selected client '" + clientName
    ...
    private Consumer<AuthContext> cacheAuthContentTypeValidator(String expectedContentType) {
        return authContext -> validateCacheAuthContentType(expectedContentType, authContext);
    }

    private static String canonicalCacheContentType(MediaType contentType) {
        if (contentType == null) {
            return null;
        }
        Map<String, String> parameters = new TreeMap<>(String.CASE_INSENSITIVE_ORDER);
        contentType.getParameters().forEach((name, value) -> parameters.put(
                name.toLowerCase(Locale.ROOT),
                "charset".equalsIgnoreCase(name) && contentType.getCharset() != null
                        ? contentType.getCharset().name()
                        : value));
        return new MediaType(contentType.getType(), contentType.getSubtype(), parameters).toString();
    }

    private MediaType effectiveCacheBodyContentType(Object body, String contentTypeHeader) {
        if (contentTypeHeader == null) {
            return body != null ? MediaType.APPLICATION_JSON : null;
        }
    ...
    private Mono<CacheBodyPreparation> prepareCacheSelectedBody(
        ...
        final String canonicalContentType;
        try {
            effectiveContentType = effectiveCacheBodyContentType(body, contentTypeHeader);
            canonicalContentType = canonicalCacheContentType(effectiveContentType);
        } catch (IllegalArgumentException error) {
            return Mono.error(new RequestSerializationException(clientName, error));
        }
        Consumer<AuthContext> authContextValidator = cacheAuthProvider != null
                ? cacheAuthContentTypeValidator(canonicalContentType)
                : null;
        if (body == null) {
            return Mono.just(new CacheBodyPreparation(
                    new SerializedRequestBody(null, null, null, authContextValidator),
                    CacheKeyContract.absentSerializedBodyKey(canonicalContentType)));
        }
        if (body instanceof byte[] bytes) {
    ...
    private static CacheBodyPreparation cacheBodyPreparation(
            ...
            Consumer<AuthContext> authContextValidator) {
        ...
        return new CacheBodyPreparation(
                new SerializedRequestBody(originalBody, stableWireBytes, stableWireBytes, authContextValidator),
    ...
    private void completeAndReportTerminal(
    ...
    private static SubscriptionReportingState subscriptionState(reactor.util.context.ContextView context) {
    ...
    private record SerializedRequestBody(
            Object originalBody,
            Object bodyToWrite,
            byte[] rawBody,
            Consumer<AuthContext> authContextValidator) {}
