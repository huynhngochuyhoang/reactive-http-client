...
- **Protocol-aware pool capacity.** Added address-free per-client pool meters that separate HTTP/1.1 pending connections from HTTP/2 active/pending streams, retained unknown peer stream limits in sanitized diagnostics, and added real one-connection H1/H2 pressure coverage for queueing, cancellation, timeout, reuse, and owned shutdown.
