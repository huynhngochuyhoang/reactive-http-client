...
- **Pre-response failure attribution.** Added bounded `DNS_RESOLUTION`, `PROXY_CONNECT`, and `TLS_HANDSHAKE` stages alongside expanded `CONNECT` evidence; preserved the existing `ErrorCategory` mapping; prevented nested auth-provider and pre-dispatch custom-filter failures from being promoted into business-request transport stages; and aligned real DNS, proxy, connect, TLS, certificate, lifecycle, logging, metrics, health, mock, support-bundle, and operations contracts.
