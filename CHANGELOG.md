...
- **Timeout phase and terminal-state parity.** Added bounded `CONNECT`, `REQUEST_WRITE`, `RESPONSE_HEADERS`, and `RESPONSE_BODY` failure-stage attribution alongside `POOL_ACQUIRE`; aligned lifecycle, exchange-log, observer, Micrometer, and OTel terminal metadata; required final-request dispatch evidence for response-header attribution; added real-wire response-header, unary-body, streaming, cancellation, envelope, precedence, and disabled-timeout contracts; and added explicit mock terminal-failure support without implying network timing simulation.
