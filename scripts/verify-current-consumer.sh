...
CACHE_DISABLED_REPORTS="$EVIDENCE_DIR/cache-disabled-surefire-reports"
FIXTURE_POM="$ROOT_DIR/.github/boot4-consumer/pom.xml"
CACHE_DISABLED_FIXTURE_POM="$ROOT_DIR/.github/boot4-cache-disabled-consumer/pom.xml"
...
mkdir -p "$LOCAL_REPOSITORY" "$EVIDENCE_DIR/effective-poms" \
  "$MOCK_REPORTS" "$CONSUMER_REPORTS" "$CACHE_DISABLED_REPORTS"
...
copy_cache_disabled_reports() {
  for report in "$ROOT_DIR/.github/boot4-cache-disabled-consumer/target/surefire-reports/"*.xml; do
    [[ -f "$report" && "$report" -nt "$REPORT_START_MARKER" ]] \
      && cp "$report" "$CACHE_DISABLED_REPORTS/"
  done
}
...
preserve_reports() {
  ...
  [[ -z "$(git -C "$ROOT_DIR" status --porcelain)" ]] || working_tree=dirty
  {
    ...
    echo "fixtures=.github/boot4-consumer/pom.xml,.github/boot4-cache-disabled-consumer/pom.xml"
...
"${MAVEN[@]}" -f "$FIXTURE_POM" -Dreactive-http-client.version="$PROJECT_VERSION" \
  ...
  -Dconsumer.v29.parity=true clean test
stage="consumer-tests"
copy_consumer_reports
"${MAVEN[@]}" -f "$FIXTURE_POM" -Dreactive-http-client.version="$PROJECT_VERSION" \
  -Dconsumer.v27.parity=true -Dconsumer.v28.parity=true -Dconsumer.v29.parity=true \
...
if grep -Eq "$ROOT_DIR/reactive-http-client-(starter|test|otel)/target/(test-)?classes" \
    "$EVIDENCE_DIR/classpath.txt" "$EVIDENCE_DIR/dependency-tree.txt" \
    "$EVIDENCE_DIR/cache-disabled-classpath.txt" "$EVIDENCE_DIR/cache-disabled-dependency-tree.txt"; then
...
grep -q "/com/github/ben-manes/caffeine/caffeine/" "$EVIDENCE_DIR/classpath.txt" \
  || fail "weighted cache consumer did not receive explicitly selected Caffeine storage"
if grep -q "/com/github/ben-manes/caffeine/caffeine/" \
    "$EVIDENCE_DIR/cache-disabled-classpath.txt" "$EVIDENCE_DIR/cache-disabled-dependency-tree.txt"; then
...
grep -q "$LOCAL_REPOSITORY/$GROUP_PATH/reactive-http-client-starter/$PROJECT_VERSION/reactive-http-client-starter-$PROJECT_VERSION.jar" \
  "$EVIDENCE_DIR/cache-disabled-classpath.txt" \
  || fail "starter jar is absent from the isolated cache-disabled consumer classpath"
stage="cache-disabled-artifact"
