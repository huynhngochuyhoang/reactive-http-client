...
Sections without a version label describe the current `3.5.0`
...
The default reactor now declares `3.5.0`, imports Spring Boot `4.0.0`,
...
from the current reactor version. The `3.5.0` release candidate compares
...
`target/release-evidence/current-consumer/current-3.5.0/`. Fresh Surefire XML is copied immediately after each successful mock or consumer
