...
published `4.4.1` when consuming it from a release repository.
...
resolve starter `4.4.1`, Boot 4 modules, Jackson 3, and Boot 4 Resilience4j
...
| OTel propagation, mock helpers, or their auto-configuration is missing | Compare the starter, `reactive-http-client-otel`, and `reactive-http-client-test` versions in the tree. | Keep all three on `4.4.1`; keep the test helper test-scoped. The OTel companion remains optional and must not be copied from the `2.x` lane. |
...
The latter is the adoption check for starter `4.4.1`; it does not consume
