...
Spring Boot 4 starter 4.x line. The current reactor is `4.4.0-SNAPSHOT`; use
published `4.3.0` when consuming it from a release repository.
...
resolve starter `4.3.0`, Boot 4 modules, Jackson 3, and Boot 4 Resilience4j
...
| OTel propagation, mock helpers, or their auto-configuration is missing | Compare the starter, `reactive-http-client-otel`, and `reactive-http-client-test` versions in the tree. | Keep all three on `4.3.0`; keep the test helper test-scoped. The OTel companion remains optional and must not be copied from the `2.x` lane. |
...
The latter is the adoption check for starter `4.3.0`; it does not consume
reactor classes or unpublished `4.4.0-SNAPSHOT` artifacts.
