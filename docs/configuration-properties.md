...
| `reactive.http.clients.[name].cache.policies.[policy-name].maximum-total-decoded-response-bytes` | `java.lang.Long` |  | Optional per-policy aggregate decoded response representation byte limit. Must be between 1 and 1099511627776 (1 TiB). This is additional to ttl-ms and maximum-size and is not a Java heap, direct-memory, RSS, or container-memory limit. |  |
