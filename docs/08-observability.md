...
When `spring-boot-starter-actuator` is on the classpath, a sanitized configured-client diagnostics endpoint is available as an explicit opt-in. It is disabled by default and uses the endpoint id `reactive-http-clients`.
