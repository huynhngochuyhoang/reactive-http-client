...
The initial implementation verification below is **dirty-tree evidence**; the
subsequent clean-commit native verification is recorded separately under Native
Gate. Development remains `4.4.0-SNAPSHOT`; published artifacts
...
**Completed 2026-09-13**, from clean commit
`f9b94fd207e6c5af1fc36ee047fd2c491a6c0e30` (tree
`da53a49e0bb646fa0709dced7e69a7626b154607`). The fixture, installed reactor
artifacts and executable all came from this revision. The tree remained clean
through the run; only these evidence/checklist notes were edited afterward.

Recorded commands from that checkout:
...
Linux x86-64-v3 and GCC `13.3.0` were used. The native build regenerated AOT
output, reran **6 JVM fixture tests** (zero failures, errors or skips), and
finished with exit zero in 8m18s Maven elapsed time. Native Image reported
8m3s image generation and 4.66 GB peak RSS; these are execution provenance,
not a performance claim. The executable exited zero within the process bound,
emitted the V31 capture/restoration marker, and completed the existing cache,
resilience, shutdown and context-recreation smoke assertions.

Executable SHA-256:
`c074e8f5bc3a340f9a8d982136fcc5b752a74c1bf9952c7156e26bd9be1fa5ed`.

Evidence resides in `target/release-evidence/v31/priority7/native-clean/`:
exact commands, start/end times, full compile/run output and exit statuses,
toolchain versions, six fresh Surefire cases, clean status before/after,
fixture source archive and checksums, and verified executable checksum.
Final provenance reports `completedStage=evidence-verified`, `exitStatus=0`.
The earlier memory preflight wait was stopped at the user's request before
native compilation and is retained separately; it was not a failed native build.
The compiler and executable each passed on their first run without source changes.
Warnings remain in the logs and are not represented as warning-free evidence.

Rebuild after any relevant source change. This fixture does not certify Istio, MVC,
