...
| Resilience absent or unselected | Factory selects operator names first, resolveSafely loads registry types by name, and resolves no-op when unavailable; auto-configuration isolates metric binders behind class/bean conditions. The corrected minimal consumer explicitly sets resilience.enabled=true, asserts the bound enabled flag and four absent operator names, and completes two GET-only loopback calls without registry classes | Enabled-only is not the default: ResilienceConfig defaults enabled to false. The 2026-09-16 correction supplies this evidence; the original base-URL-only run did not. No arbitrary partial/mismatched Resilience4j artifact matrix. Existing full-classpath registry-absence/selection cases are different from removal of individual operator classes |
